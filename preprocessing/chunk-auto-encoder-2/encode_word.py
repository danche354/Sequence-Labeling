...
for i, word in enumerate(all_word):
    w.write(word+'\n')
    word_hashing = prepare.prepare_auto_encoder(batch=[word], task='chunk', gram='bi')
...
embeddings.to_csv('../preprocessing/chunk-auto-encoder-2/auto-encoder-embeddings.txt', sep=' ',header=False,index=False,float_format='%.6f')
w.close()
