...
nb_epoch = 50 # nb_epoch = conf.nb_epoch
...
train_data, dev_data = load_data.load_chunk(dataset='train.txt', split_rate=split_rate, amount=10)
...
for epoch in range(nb_epoch):
    ...
    for i in range(number_of_train_batches):
        ...
        embed_index, hash_repesentation, pos, label, length, sentence = prepare.prepare_chunk_raw(batch=train_batch, gram='bi')
    ...
    for j in range(number_of_dev_batches):
        ...
        embed_index, hash_repesentation, pos, label, length, sentence = prepare.prepare_chunk_raw(batch=dev_batch, gram='bi')
