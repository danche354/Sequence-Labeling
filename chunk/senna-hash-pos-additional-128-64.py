...
additional_length = conf.chunk_additional_length
...
addtional_input = Input(shape=(step_length, additional_length))

senna_hash_pos_merge = merge([embedding, encoder_embedding, pos_input, addtional_input], mode='concat')
...
model = Model(input=[embed_index_input,hash_index_input,pos_input, addtional_input], output=output)
...
for epoch in range(nb_epoch):
    ...
    for i in range(number_of_train_batches):
        ...
        train_metrics = model.train_on_batch([embed_index, hash_index, pos, additional], y)
    ...
    for j in range(number_of_dev_batches):
        ...
        y = np.array([np_utils.to_categorical(each, output_length) for each in label])
        # for loss
        dev_metrics = model.test_on_batch([embed_index, hash_index, pos, additional], y)
