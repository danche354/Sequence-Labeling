...
senna_pos_chunk_gazetteer_merge = merge([embedding, pos_input, gazetteer_input], mode='concat')
...
model = Model(input=[embed_index_input,pos_input, gazetteer_input], output=output)
...
#        chunk = np.array([(np.concatenate([np_utils.to_categorical(c, chunk_length), np.zeros((step_length-length[l], chunk_length))])) for l,c in enumerate(chunk)])
        ...
        train_metrics = model.train_on_batch([embed_index, pos, gazetteer], y)
...
#        chunk = np.array([(np.concatenate([np_utils.to_categorical(c, chunk_length), np.zeros((step_length-length[l], chunk_length))])) for l,c in enumerate(chunk)])
        ...
        y = np.array([np_utils.to_categorical(each, output_length) for each in label])
        # for loss
        dev_metrics = model.test_on_batch([embed_index, pos, gazetteer], y)
        dev_loss += dev_metrics[0]

        # for accuracy
        prob = model.predict_on_batch([embed_index, pos, gazetteer])
