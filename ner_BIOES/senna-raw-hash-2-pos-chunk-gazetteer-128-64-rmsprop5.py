...
senna_hash_pos_chunk_gazetteer_merge = merge([embedding, hash_representation_input, pos_input, chunk_input, gazetteer_input], mode='concat')
...
model = Model(input=[embed_index_input,hash_representation_input,pos_input,chunk_input, gazetteer_input], output=output)
...
for epoch in range(nb_epoch):
    ...
    for i in range(number_of_train_batches):
        ...
        train_metrics = model.train_on_batch([embed_index, hash_repesentation, pos, chunk, gazetteer], y)
    ...
    for j in range(number_of_dev_batches):
        ...
        y = np.array([np_utils.to_categorical(each, output_length) for each in label])
        # for loss
        dev_metrics = model.test_on_batch([embed_index, hash_repesentation, pos, chunk, gazetteer], y)
        dev_loss += dev_metrics[0]

        # for accuracy
        prob = model.predict_on_batch([embed_index, hash_repesentation, pos, chunk, gazetteer])
